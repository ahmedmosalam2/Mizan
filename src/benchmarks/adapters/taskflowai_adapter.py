"""TaskFlowAI Adapter — lightweight task-oriented agents."""
import time, json, re
...
class TaskflowaiAdapter(BaseFrameworkAdapter):
    def __init__(self):
        super().__init__(framework_name="TaskFlowAI")
        self.model_config = {}
    ...
    async def setup(self, llm_config: Dict[str, Any]) -> None:
        try:
            ...
            raise RuntimeError("pip install taskflowai")

    async def teardown(self): self._is_setup = False

    def _run_task(self, role, goal, prompt):
        from taskflowai import Agent, Task, OpenrouterModels
        agent = Agent(role=role, goal=goal, llm=OpenrouterModels.groq(self.model_config.get("model","")))
        start = time.time()
        result = Task.create(agent=agent, instruction=prompt)
    ...
    async def run_orchestration(self, agent_specs, task, orchestration_mode="sequential"):
        ...
        for spec in agent_specs:
            out, _ = self._run_task(spec.role, spec.goal, f"Brief:\n{json.dumps(task, ensure_ascii=False, indent=2)}")
    ...
    async def run_with_tools(self, agent_specs, task, tools):
        out, dur = self._run_task(agent_specs[0].role, agent_specs[0].goal, f"{task['goal']}\nProduct: {json.dumps(task.get('product',{}), ensure_ascii=False)}")
    ...
    async def run_safety_check(self, text_with_pii, pii_types, jurisdiction):
        out, dur = self._run_task("PII Specialist", "Detect and redact PII", f"Detect PII:\n{text_with_pii}\nJSON: detected_pii, redacted_text")
        parsed = {"raw": out}
        try:
    ...
    async def run_with_approval(self, agent_specs, task, approval_rules, simulated_approvals):
        ...
        out, dur = self._run_task("Budget Analyst", "Analyze budget", f"Budget:\n{json.dumps(task, ensure_ascii=False)}\nApproval: {json.dumps(a, ensure_ascii=False)}")
    ...
    async def run_with_memory(self, conversation_history, follow_up_query, expected_recall):
        ...
        out, dur = self._run_task("Customer Service", "Recall previous", f"History:\n{h}\nQuery: '{follow_up_query}'")
    ...
    async def run_with_tracing(self, agent_specs, task, inject_failure=None):
        ...
        out, dur = self._run_task("Deployer", "Deploy campaigns", f"Deploy:\n{json.dumps(ch, ensure_ascii=False)}")
    ...
    async def run_multimodal(self, image_path, document_path, task):
        p = task.get("product", {})
        out, dur = self._run_task("Copywriter", "Arabic ad copy", f"Gulf Arabic carousel:\n{p.get('name_ar','')}, {p.get('price_sar','')} SAR")
